...
public class BlogMockedData {
    ...
    public Blog getBlogById(int id){
        for (Blog searchedBlog: blogs){
            if (searchedBlog.getId() == id){
                return searchedBlog;
            }
        }
        return null;
    ...
    public List<Blog> searchBlogs(String text) {
        ...
        for (Blog searchedBlog: blogs){
            if (searchedBlog.getTitle().toLowerCase().contains(text.toLowerCase()) || searchedBlog.getContent().toLowerCase().contains(text.toLowerCase())){
                searchedBlogs.add(searchedBlog);
            }
    ...
    public Blog updateBlog(int id, String title, String content){
        for (Blog updatedBlog: blogs){
            if (updatedBlog.getId() == id){
                int blogIndex = blogs.indexOf(updatedBlog);
                updatedBlog.setTitle(title);
                updatedBlog.setContent(content);
                blogs.set(blogIndex, updatedBlog);
                return updatedBlog;
            }
        } return null;
    }
    ...
    public boolean deleteBlog(int id){
        ...
        for (Blog blogToDelete: blogs){
            if (blogToDelete.getId() == id) {
                blogIndex = blogs.indexOf(blogToDelete);
                continue;
            }
        }
        if (blogIndex > -1){
